...
def validate_compatible_path(path):
    ...
    if real_rel != relpath:
        raise InvalidCase("%s is mis-capitalised on disk as %r.\nYou should rename it to be " "correctly lowercase, for cross-platform portability." % (relpath, real_rel))
...
class ResourceLoader:
    ...
    def validate_root(self, name):
        ...
        if self._have_root:
            ...
            raise KeyError("No '{subpath}' directory found to load {type} " "'{name}'.".format(subpath=self._subpath, type=self.TYPE, name=name))
    ...
    def load(self, name, *args, **kwargs):
        ...
        if not os.path.isfile(p):
            for ext in self.EXTNS:
                ...
                raise KeyError("No {type} found like '{name}'. " "Are you sure the {type} exists?".format(type=self.TYPE, name=name))
